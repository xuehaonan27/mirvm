//! The file interface, which is the C library's on every platform this build supports.
//!
//! The descriptor half is the `read`/`write`/`pipe`/`close` family and the path half is publishing,
//! linking, mode bits and metadata identity. Splitting them would put one libc call per module and
//! leave the porting surface larger than the knowledge in it.
//!
//! Each of these is a sequence a caller would otherwise have to spell out: writing at an offset
//! through a vector, or asking whether two metadata reads describe the same unchanged file.
//!
//! The one thing POSIX does not give is a rename that refuses an existing destination while
//! replacing it atomically, so `rename_noreplace` is the platform's and is re-exported from here.
...
#[cfg(target_os = "linux")]
pub(crate) use super::linux::fs::rename_noreplace;
#[cfg(target_os = "macos")]
pub(crate) use super::macos::fs::rename_noreplace;

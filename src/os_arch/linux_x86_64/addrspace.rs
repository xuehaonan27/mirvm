//! The fixed-address layout of this pair: which band of Linux/x86_64's virtual address space the
//! engine's frozen data regions and code regions occupy.
//!
//! The *structure* is what the engine asks of every pair (P1/P2/S3'/S4 address models,
//! decision-history §7.5b/§7.6/§7.5c/§7.3): three frozen data regions — base image, delta, and a
//! spline of dependency images — plus three isomorphic, disjoint code-region families, all at
//! fixed bases so that absolute addresses are stable across processes and the IR cache keeps
//! working. The *numbers* are this pair's, because they are chosen against this kernel's
//! address-space layout. Arena implementations in `vm/frozen.rs` and `vm/codearena.rs` use this
//! as the source of truth; the IR serialization whitelists, the image layers' load validation,
//! and lower assembly all share the same numeric values; no second literal is allowed.
//!

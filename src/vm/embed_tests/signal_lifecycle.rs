...
fn cross_engine_synchronous_signal_fault_returns_to_the_raising_boundary() {
    ...
    let test_name = "vm::embed_tests::signal_lifecycle::cross_engine_synchronous_signal_fault_returns_to_the_raising_boundary";
    let mut failures = Vec::new();
    for (mode, _) in modes() {
...
fn cross_engine_mask_restored_signal_fault_returns_to_the_raising_boundary() {
    ...
    let test_name = "vm::embed_tests::signal_lifecycle::cross_engine_mask_restored_signal_fault_returns_to_the_raising_boundary";
    let mut failures = Vec::new();
    for (mode, _) in modes() {
        let output = std::process::Command::new(std::env::current_exe().unwrap())
            .args(["--exact", test_name, "--nocapture", "--test-threads=1"])
...
fn faulting_handler_same_signal_raise_survives_cross_thread_close() {
    ...
    let test_name = "vm::embed_tests::signal_lifecycle::faulting_handler_same_signal_raise_survives_cross_thread_close";
    let mut failures = Vec::new();
    for (mode, _) in modes() {
        let output = std::process::Command::new(std::env::current_exe().unwrap())

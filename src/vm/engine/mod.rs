//! M4 engine (real): typed bytecode + byte-region frames + typed interp_frame.
//! Pure Rust, zero rustc_private — same-source compilation via the tsan harness is the mechanical gate for execution-phase purity.
//! The load phase (lowering MIR to this IR) lives in src/lower/ (rustc_private domain); its product is handed off via `ir::Module`.

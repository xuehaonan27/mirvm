...
fn constructor_signal_fault_drains_masked_reraise_before_failed_engine_closes() {
    ...
    let test_name = "vm::engine::embed_tests::engine_lifecycle::constructor_signal_fault_drains_masked_reraise_before_failed_engine_closes";
    let mut failures = Vec::new();
    for (mode, _) in modes() {
...
fn finalizer_engine_fault_aborts_inside_teardown_boundary() {
    ...
    let test_name = "vm::engine::embed_tests::engine_lifecycle::finalizer_engine_fault_aborts_inside_teardown_boundary";
...
fn closed_plain_c_p1_entry_aborts() {
    ...
    let test_name = "vm::engine::embed_tests::engine_lifecycle::closed_plain_c_p1_entry_aborts";
    let output = std::process::Command::new(std::env::current_exe().unwrap())
        .args(["--exact", test_name, "--nocapture"])
        .env(CHILD, "1")
        .output()
...
fn engine_closed_obeys_a_guest_terminate_boundary() {
    ...
    let test_name =
        "vm::engine::embed_tests::engine_lifecycle::engine_closed_obeys_a_guest_terminate_boundary";
    let output = std::process::Command::new(std::env::current_exe().unwrap())
        .args(["--exact", test_name, "--nocapture"])
        .env(CHILD, "1")
        .output()

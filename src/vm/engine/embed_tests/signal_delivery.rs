...
fn physically_blocked_native_finalizer_raise_survives_close_worker_exit() {
    ...
    let test_name = "vm::engine::embed_tests::signal_delivery::physically_blocked_native_finalizer_raise_survives_close_worker_exit";
...
fn sigwaitinfo_consumes_blocked_host_raise_without_leaving_thread_signal_state() {
    ...
    let test_name = "vm::engine::embed_tests::signal_delivery::sigwaitinfo_consumes_blocked_host_raise_without_leaving_thread_signal_state";
...
fn raw_pthread_kill_runs_only_on_the_target_pthread_during_concurrent_close() {
    ...
    let test_name = "vm::engine::embed_tests::signal_delivery::raw_pthread_kill_runs_only_on_the_target_pthread_during_concurrent_close";
    let mut child = std::process::Command::new(std::env::current_exe().unwrap())
        .args(["--exact", test_name, "--nocapture", "--test-threads=1"])
        .env(CHILD, "1")
        .stdout(std::process::Stdio::piped())
...
fn wait_closed_fails_fast_for_a_signal_pending_on_the_current_pthread() {
    ...
    let test_name = "vm::engine::embed_tests::signal_delivery::wait_closed_fails_fast_for_a_signal_pending_on_the_current_pthread";
    let mut child = std::process::Command::new(std::env::current_exe().unwrap())
        .args(["--exact", test_name, "--nocapture", "--test-threads=1"])
        .env(CHILD, "1")
        .stdout(std::process::Stdio::piped())
...
fn external_siginfo_handler_can_wait_for_another_threads_host_signal() {
    ...
    let test_name = "vm::engine::embed_tests::signal_delivery::external_siginfo_handler_can_wait_for_another_threads_host_signal";
    let mut child = std::process::Command::new(std::env::current_exe().unwrap())
        .args(["--exact", test_name, "--nocapture", "--test-threads=1"])
        .env(CHILD, "1")
        .stdout(std::process::Stdio::piped())
...
fn external_siginfo_handler_can_wait_for_another_threads_engine_close() {
    ...
    let test_name = "vm::engine::embed_tests::signal_delivery::external_siginfo_handler_can_wait_for_another_threads_engine_close";
    let mut child = std::process::Command::new(std::env::current_exe().unwrap())
        .args(["--exact", test_name, "--nocapture", "--test-threads=1"])
        .env(CHILD, "1")
        .stdout(std::process::Stdio::piped())

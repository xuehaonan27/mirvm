...
rand_chacha = "0.9"
---
// rand 0.9 + rand_chacha 0.9：定种 ChaCha8/ChaCha20 双流抽取。
// 覆盖 seed_from_u64、u64/f64 抽取（f64 打 to_bits 锁位型）、shuffle、
// random_range 多整数区间、random_bool/random_ratio 固定次数比例统计、
// random_iter/sample_iter、Uniform 分布及错误路径。输出全确定。
...
fn main() {
    ...
    // ⑥ random_iter / sample_iter（random_sample 在 rand 0.9 不存在，0.10 才有）

...
报错，不能忽略参数后继续运行。当前明确不在范围内的是 doctest、交叉 target，以及
...
doctest 需要 rustdoc 前端，不属于普通 test harness。
...
开发依赖、unit test 和 integration 展开。工作区夹具 `tests/fixtures/cless_workspace_contract` 采用
resolver 3，并覆盖

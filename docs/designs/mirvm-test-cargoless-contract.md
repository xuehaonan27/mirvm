...
> 生效日期：2026-08-08。这里的 “cargoless” 指 mirvm 自己解析清单、安排编译，
> 运行过程中不启动 Cargo。实现事实以代码和
> `tests/cargoless_test_contract.sh` 为准。
...
合同只承诺已经支持的单包范围。范围内必须对齐；范围外必须明确报错，不能忽略参数后
继续运行。当前明确不在范围内的是 workspace/package 选择、doctest、bench、根包
feature 选择、交叉 target 和根 proc-macro 包测试。其中 workspace/package 归 D15 P5，
doctest 需要 rustdoc 前端；其余在真实项目需要时扩合同。
...
固定夹具 `tests/fixtures/cless_test_contract` 同时覆盖库测试、bin 测试、integration
test、普通 example、自定义 harness、build script、path 开发依赖、panic、ignored、
失败传播、bin 子进程和 test profile。

`tests/cargoless_test_contract.sh` 只保留三层判定：
...
3. **机制层**：self 腿把 PATH 中的 `cargo` 替换成必失败哨兵，并在 Linux 基线上用
   ...
   执行次数仍为 1，防止“结果相同但偷偷退回 Cargo”或增量失效。
...
3. 行为变化先改 manifest/resolve/schedule/driver 和对应单测，再改合同断言；不允许
   仅放宽归一化规则让测试变绿。
4. 依次运行 `cargo test --locked`、本合同、`tests/diff_cless.sh`，最后跑
   `tests/run.sh fast`。三轨全部通过后，新的 pinned Cargo 才成为权威。

...
| `modeb-mirvmar-design.md` | mode B：`.mirvm` 包格式 v2 + pack/run/MC（已落地；格式声明不定死） |
| `product-capabilities-plan.md` | 明确缺失的产品能力与建议施工顺序（规划记录，尚未逐项开工） |
| `mirvm-test-cargoless-contract.md` | `mirvm test` 单包 self/compat/Cargo 三轨合同、明确边界与 Cargo 升级规程 |

...
| D15 | **砍掉 cargo（自有依赖解析 + 编译调度；用户 2026-07-22 纳入日程）** | **P1-P4、resolver 2/3 常见 workspace 与 rust-version-aware 选择已完成**（2026-07-27 至 08-10，decision-history §7.28-§7.36）：manifest/lock/registry/resolve、自有编译调度、build.rs 生命周期、rustflags、增量、并行、sysroot 自管、默认 self、成员发现/继承/统一 lock/package 与 feature 选择、resolver 3、fallback/allow、`--ignore-rust-version`、lock v3/v4 均已落地并通过 Cargo/compat/self 合同。package `[lints]` 的 level/priority/check-cfg 也已进入 rustc 参数和编译指纹。**P5 仍开放**：git 源、alt registry/config、source replacement/patch、resolver 1、复杂成员 glob/嵌套 workspace/workspace lints；均须响亮拒绝，不能静默退化。其他边界：HOST_RUSTFLAGS、完整 Cargo config 合并、`mirvm pack` 翻 self 尚未做 | decision-history §7.22/§7.27-§7.36，[d15-cargoless-design.md](designs/d15-cargoless-design.md)，[mirvm test 合同](designs/mirvm-test-cargoless-contract.md) |

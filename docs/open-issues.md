...
| D15 | **砍掉 cargo（自有依赖解析 + 编译调度；用户 2026-07-22 纳入日程）** | **P1-P4 与 resolver 2 常见 workspace 已完成**（2026-07-27 至 08-08，decision-history §7.28-§7.35）：manifest/lock/registry/resolve、自有编译调度、build.rs 生命周期、rustflags、增量、并行、sysroot 自管、默认 self、成员发现/继承/统一 lock/package 与 feature 选择均已落地并通过 Cargo/compat/self 合同。**P5 仍开放**：git 源、alt registry、source replacement、resolver 1/3 与 rust-version-aware 选择、复杂成员 glob/嵌套 workspace/workspace lints；均须响亮拒绝，不能静默退化。其他边界：HOST_RUSTFLAGS、config 多文件合并、`mirvm pack` 翻 self 尚未做 | decision-history §7.22/§7.27-§7.35，[d15-cargoless-design.md](designs/d15-cargoless-design.md)，[mirvm test 合同](designs/mirvm-test-cargoless-contract.md) |
...
| D17 | **mirvm test（用户 2026-07-29 立项）** | **单包主链与 resolver 2 常见 workspace 已完成（2026-08-08）**：除既有 lib/bin/integration/example/custom harness、Dev 依赖和 test profile 外，现支持默认/当前成员、`--workspace`/`--all`、`-p`/`--package`、`--exclude`、默认/指定/依赖 feature、继承、统一 lock 与多包 fail-fast；单包合同 20/20、workspace 合同 27/27，self 腿由 PATH 哨兵 + execve 审计证明零 Cargo。**仍缺**：bench、根 proc-macro；doctest 继续明确不做，需 rustdoc 专项。resolver 1/3 和复杂依赖来源归 D15 | [mirvm test 合同](designs/mirvm-test-cargoless-contract.md)，decision-history §7.32/§7.34/§7.35 |

...
> 覆盖 mirvm 自始（M0 tier-0 时代）至 2026-08-13 当前复核的
...
| D16 | **统一性能战役（用户 2026-07-29 立项，当前延期）** | 恢复时先跑 `MIRVM_TIMING` 相位账本、dev 循环改一行重跑，并按日志/profile 审计稿的真实 workload 预算复测，不从旧数字猜收益。主候选按裁定序：①**D5/L3 JIT 码持久化**；②D3 已完成的逐函数驻留/热序预取与 v4 owned snapshot 整包复制实测；③后台服务线程；④D12 `-Cincremental`。原 E6/E7/E30/E37 统一在本战役内裁判：分配与 guest TLS 助手内联（旧计量 alloc 7.82M / tls_ref 0.38M，进场时触发 vmctx T/R 复测）、SwitchInt/取址逃逸/间接调用/LSDA/SIMD/PLT 等 JIT 候选、C7 regex 旧 42s 靶子的 JIT 后复测，以及日志/事件/profile 的真实纵切。已知未复核红线是热缓存 `fib(32)` 约 97ms > 80ms；不得放宽门槛关账。**2026-08-13 现状纠偏：仓库只有 `mirvm_log!` 宏和 `log` facade 转发，没有 mirvm 自有 backend、`MIRVM_LOG` 读取或 `trace-log` feature；旧“日志 v1/v2”说法及纳秒估算不得当作实现事实。**ring/writer 已按审计稿逐项重定；首个真实纵切是无内部时间戳、成对提交的 MIRVM-owned `SyscallEnter`/`SyscallExit`，旧 `MIRVM_SYSCALL_TRACE` 只作债务基线，完整进程 syscall 与 duration 另走内核 raw-syscall 流；libc Exit 只在失败时读取 attach 阶段缓存的 `errno_ptr`，producer 全路径不得修改 errno；Enter 最终 drop 后整对锁定，返回点不抢救孤立 Exit。性能基线按维护者要求另期统一恢复；线程原则仍是 guest 优先、demand > 预测序 > 闲时回填，模式 A 懒降低维持否决 | designs/m5-design.md §3，designs/m5.4-design.md，designs/mirvm_high_performance_log.md，decision-history §7.20/§7.32/§7.33/§7.45/§7.53/§7.56 |

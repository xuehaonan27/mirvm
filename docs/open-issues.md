...
| E36 | **cargo 项目模式 guest cwd=项目目录，与 cargo run 语义分叉**（2026-07-23 测试管线整顿实锤） | **self 路径已闭合（D15 P2，decision-history §7.29）**：cargoless driver 全程不 chdir——guest cwd=调用者 cwd，与 cargo run（含 --manifest-path）语义一致。**compat 路径（cargo 三阶段）行为照旧**：phase_cargo 以 current_dir=项目目录驱动 cargo，runner 继承之 → guest cwd=项目目录（`mirvm run <项目目录>` 时 guest cwd=项目目录；而 `cargo run` 从不 chdir——argv 探针实锤两语义分叉；corpus/projects 对拍以 {ROOT} 绝对化夹具路径绕行，harness 层合法）。P4 默认翻 self 后默认路径即对齐；compat 删除评审时全闭 | tests/corpus.manifest {ROOT} 注，decision-history §7.26/§7.29 |
...
| D15 | **砍掉 cargo（自有依赖解析 + 编译调度；用户 2026-07-22 纳入日程）** | **P1-P4 已完成**（2026-07-27 至 07-29，decision-history §7.28-§7.31）：manifest/lock/registry/resolve、自有编译调度、build.rs 生命周期、rustflags、增量、并行、sysroot 自管与默认 self 均已落地并通过 corpus 双轨。**2026-08-08 合同加严**：依赖声明 feature 全集与启用集分离，所有 rustc 单元的 check-cfg 对齐 Cargo；根 Dev 边只进测试单元；test profile 的 build-script env 对齐。**P5 仍开放**：git 源、alt registry、workspace 多包图、source replacement、rust-version-aware 选择；均须响亮拒绝，不能静默退化。其他边界：HOST_RUSTFLAGS、config 多文件合并、`mirvm pack` 翻 self 尚未做 | decision-history §7.22/§7.27-§7.31，[d15-cargoless-design.md](designs/d15-cargoless-design.md)，[mirvm test 合同](designs/mirvm-test-cargoless-contract.md) |
...
| D17 | **mirvm test（用户 2026-07-29 立项）** | **单包主链已完成（2026-08-08）**：lib/bin/integration/example/custom harness、cfg(test)、Dev 依赖、test profile、目标选择、过滤与 harness 参数、no-run/fail-fast/quiet/locked/offline、独立进程、失败码、CARGO_BIN_EXE 与热构建均已落地；Cargo/compat/self 合同 20 判定全绿，self 腿由 PATH 哨兵 + execve 审计证明零 Cargo。**仍缺**：workspace 根与 `-p` 选择（先依赖 D15 P5 多包图）、根 feature CLI、bench、根 proc-macro；doctest 继续明确不做，需 rustdoc 专项 | [mirvm test 合同](designs/mirvm-test-cargoless-contract.md)，decision-history §7.32 |

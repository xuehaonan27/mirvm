...
| E6 | **分配快路径/guest TLS 快路径内联未做** | `未立项` m5-design D5 格③空（分配走 mirvm_alloc 助手、TlsRef 走助手）；手卷 TLAB 同题（E14）。**2026-07-19 身份升格 = vmctx 复测双闸之闸①**（vmctx-passing §7；立项时以格③真实负载复测 T vs R，对照基线 = §7.20 计量：corpus alloc 7.82M / tls_ref 0.38M） | designs/m5-design.md §3，decision-history §7.20 |

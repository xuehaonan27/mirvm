...
| C2 | **native-archive 闭包缺口：符号在 rlib**（旧 debt §10①；**评审定案 2026-07-18，待施工**） | 评审结论 = 原理可完全闭合，路线 = 链接期注入 rlib 导出 fn 的 P1 stub 隐藏跳板（失败救援链：ld undefined → exported_defs → fn_entry_addr 预算 → `.hidden` trampoline → 重链；首链成功路径零变化）。方案与验收矩阵（MRE 转正/bzip2 C 后端复绿/c_wasmtime_wat 换面）见 [designs/c2-rlib-symbols-design.md](designs/c2-rlib-symbols-design.md)；数据符号如实拒绝另立 | designs/c2-rlib-symbols-design.md |
...
| E17 | **L2 缓存两处** | `未立项` ①有告警/错误的会话拒入账、诊断回放未做（告警程序永不享缓存，session 门在 src/cli.rs:534）；②条目无逐出无 GC（手动清 `~/.cache/mirvm/ir`） | history/m6-log.md 片2/8 |

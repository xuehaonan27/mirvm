...
| D7 | **frontend 相成本无杠杆认领；V5 `-Zthreads` 并行 lower 未立项** | eco ~143ms / ripgrep ~730-800ms 在账无人认领；V5（tcx DynSync+worklist rayon 化）在 V3 不建后无人重启。**2026-09-19 已出上游进展调研 + 改动面评估**（轴 A `-Zthreads` 注入 / 轴 B 并行预取 / 轴 C 完全体，含 bump pin 检查单），待裁定立项 | history/coldstart-research.md，m6-log 片8/10，[history/parallel-frontend-research.md](history/parallel-frontend-research.md) |

...
| D15 | **砍掉 cargo（自有依赖解析 + 编译调度；用户 2026-07-22 纳入日程）** | **P1 完成（2026-07-27，decision-history §7.28）**：`src/cargoless/` 解析地基落地——manifest/lockfile/registry/resolve/audit 五件 + `mirvm deps audit`（项目等值对账 + 脚本 cargo `--locked --offline` 验收链）；cargo 解析语义全实证定稿（resolve ∪ build 双图、lazy-bucket 多版本、optional 门按（父包,父版本,依赖键）、?/ 弱引用级联、pre 精确规则、exact 兼容 build、lock canonical、同名多 req 分立、rename 双路）。**P2 待施**：调度 + build.rs 全生命周期 + proc-macro host 编译（151 个 crate 实锤 build.rs 普遍性）→ corpus smoke 24 零 cargo 跑通；P3 全量迁移 + 双轨 gate；P4 sysroot 自管 + 默认翻转；P5 复杂语义按实需（git 源/alt registry/workspace 多包图/source replacement，响亮拒绝在案）。边界记账：rust-version-aware 版本偏好未实现（与 D14 store 合并评审时补） | decision-history §7.22/§7.27/§7.28，[designs/d15-cargoless-design.md](designs/d15-cargoless-design.md) |

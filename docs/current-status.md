...
2. **当前战役**：稳定化已收口（§7.21，2026-07-22）。**下一战役 = mode B
   （D1）**：方向已定（§7.22——打包期产字节码+机器码节，运行期读自有包 +
   FFI 真外国库）；C4 片①（dep 编译期抽取 global_asm）为其先行。**砍
   cargo（D15）已纳入日程**，之后要做。E6 性能轴继续待固定 workload 收益
   证据。

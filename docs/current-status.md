...
1. **产品能力顺序**（2026-08-10 汇总）：`mirvm test` 的当前单包/workspace 合同和
   依赖 P5 五个施工批次已完成；下一步是 P2 总 corpus 验收 → D17 余项复评 → OS 级

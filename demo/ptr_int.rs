// 真实地址内存验收：ptr↔int 往返、真对齐、wildcard 指针解引用
fn main() {
    // Box 裸指针经 usize 往返后读写
    let b = Box::new(0xABCDu64);
    let addr = Box::into_raw(b) as usize;
    println!("u64 对齐: {}", addr % align_of::<u64>() == 0);
    let back = addr as *mut u64; // int2ptr（wildcard provenance）→ 解引用
    unsafe { *back += 1 };
    println!("往返读写: {:#x}", unsafe { *back });
    drop(unsafe { Box::from_raw(back) });

    // 高对齐分配的真实对齐（Box<[u8]> 时代会假；MirvmAllocBytes 必须真）
    let v = vec![1u128; 4];

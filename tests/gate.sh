...
#
# 用法：bash tests/gate.sh                  # 全量（D15 P4 默认翻转后 = self 轨：
#                                           #   corpus ① 走零 cargo 自有调度）
#       MIRVM_DEPS=cargo bash tests/gate.sh # compat 轨：corpus ① 走 cargo 三阶段
#                                           # （② diff_cargo 恒为 cargo compat 轨，不受开关影响）

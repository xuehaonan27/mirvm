#!/usr/bin/env bash
# tests/perf.sh —— 时间性能硬门 + 空间/cache 资源计量（2026-07-23 自 gate ③ 段抽出）。
#
...
# 资源计量（总是执行）：
#   cache 各分部体量、磁盘可用、target 预算闸（tests/lib.sh）。
#
# 用法：bash tests/perf.sh                 # 硬门 + 计量
#       bash tests/perf.sh --metrics-only  # 只计量（gate 在 SKIP_PERF 时调用）
#       SKIP_PERF=1 bash tests/perf.sh     # 打 SKIP 行 + 计量，exit 0
set -u
cd "$(dirname "$0")/.."

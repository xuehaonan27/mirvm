...
are documented in the manifest header and in `tests/lib/harness.sh`.
...
`needs={DATA}/projects/<name>/Cargo.toml`, so the corpus batch records them as SKIP until the
...
1. Add it as a submodule under `tests/projects/<name>`, pinned to a commit whose output for a fixed
   ...
   hide nondeterminism.
2. Register it in `tests/manifest` as a `project-diff` case, `gate` tier first, plus
   `needs={DATA}/projects/<name>/Cargo.toml`.
3. Add its pin to the list above.
4. Verify with `make case C=<name>`.

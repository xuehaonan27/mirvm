...
# gate.sh 确实调用 runtime_gates/probes/perf/diff 家族/a2 各叶，
# 以及子 gate 的 SKIP 不会冒充 PASS。
# 全部使用确定性 fake runner，不构建项目。
# （2026-07-23 整顿：m4_gate0/1/2/4 → runtime_gates.sh，m51_* → probes.sh，
#   m4_gate5+m5_gate6 → gate.sh；检查语义不变。）
set -u
cd "$(dirname "$0")/.."
...
if MIRVM="$FIX/fake_mirvm.sh" TOOLCHAIN=gate-truth-nightly \
    ...
    /bin/bash tests/runtime_gates.sh pure >"$TMP/pure.out" 2>&1 \
    && grep -Fxq \
        'cargo +gate-truth-nightly build --manifest-path tsan/Cargo.toml --release --locked' \
        "$purity_log" \
    && ! grep -Fq 'tests/spike4_tsan.sh' "$purity_log"; then
    ok 'runtime_gates pure 纯度门只编译 harness，不再隐式重复 TSan'
else
    bad 'runtime_gates pure 仍隐式执行 TSan 或未构建纯度 harness'
    cat "$TMP/pure.out"
...
if CORPUS_PROGS=volatile MIRVM="$FIX/fake_mirvm.sh" SKIP_TSAN=1 \
    GATE_CALL_LOG="$call_log" PATH="$FIX:$PATH" /bin/bash tests/gate.sh \
    >"$TMP/gate.out" 2>&1; then
    missing=0
    for leaf in tests/runtime_gates.sh tests/probes.sh tests/perf.sh \
        tests/diff.sh tests/diff_cargo.sh tests/a2_deps_image.sh; do
        grep -Fxq "$leaf" "$call_log" || { missing=1; echo "缺调用: $leaf"; }
    done
    if [ "$missing" -eq 0 ]; then
        ok 'gate 调用 runtime_gates + probes + perf + diff 家族 + a2'
    else
        bad 'gate 缺少叶脚本调用'
        cat "$call_log"
    fi
...
if grep -Fq 'SKIP runtime_gates' "$TMP/gate.out" \
    && ! grep -Fq 'PASS runtime_gates' "$TMP/gate.out" \
    && grep -Eq '1 skip, 0 fail$' "$TMP/gate.out"; then
    ok 'gate 将 TSan SKIP 独立记作 SKIP'
else
...
if CORPUS_PROGS=volatile MIRVM="$FIX/fake_mirvm.sh" M51_SKIP_PROBE=simd_shift \
    GATE_CALL_LOG="$call_log" PATH="$FIX:$PATH" /bin/bash tests/gate.sh \
    >"$TMP/gate-probe-skip.out" 2>&1 \
    && grep -Fq 'SKIP m51_simd_shift: host lacks required CPU feature' \
        "$TMP/gate-probe-skip.out" \
    && ! grep -Fq 'PASS m51_simd_shift' "$TMP/gate-probe-skip.out" \
    && grep -Eq '1 skip, 0 fail$' "$TMP/gate-probe-skip.out"; then
...
if CORPUS_PROGS=volatile MIRVM="$FIX/fake_mirvm.sh" M51_SKIP_VECTOR_FEATURE=sha \
    GATE_CALL_LOG="$call_log" PATH="$FIX:$PATH" /bin/bash tests/gate.sh \
    >"$TMP/gate-vector-partial.out" 2>&1 \
    ...
    && ! grep -Fxq 'PASS m51_x86_vectors' "$TMP/gate-vector-partial.out" \
    && grep -Eq '1 skip, 0 fail$' "$TMP/gate-vector-partial.out"; then
...
if CORPUS_PROGS=volatile MIRVM="$FIX/fake_mirvm.sh" SKIP_TSAN=1 SKIP_PERF=1 \
    GATE_CALL_LOG="$call_log" PATH="$FIX:$PATH" /bin/bash tests/gate.sh \
    >"$TMP/gate-skip-perf.out" 2>&1 \
    && grep -Fq 'SKIP 性能上限（SKIP_PERF=1；语义 gate 仍照常执行）' \
        "$TMP/gate-skip-perf.out" \
    && grep -Eq '2 skip, 0 fail$' "$TMP/gate-skip-perf.out"; then

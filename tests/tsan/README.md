# tests/tsan/ — the execution phase under ThreadSanitizer
...
does not cover it. `rustfmt --edition 2024 tests/tsan/src/cases/*.rs tests/tsan/src/product_adapters.rs`
works; `tests/tsan/src/main.rs` and `tests/tsan/src/telemetry.rs` must be checked with
...
3. Add the id to `EXPECTED` in `tests/suites/runtime/tsan.sh`.

// Permanent differential probe for the scalar-intrinsic delta set: fabs generic name (drifts on this nightly),
...
fn float_math() {
    // fabs: generic-name intrinsic on this nightly
    println!("abs = {} {} {}", (-3.5f64).abs(), (-2.25f32).abs(), (-0.0f64).abs());
    println!("signum/max/copysign = {} {} {}",
        (-7.0f64).signum(), 1.5f32.max(2.5), 3.0f64.copysign(-1.0));
    // fma fusion proof: x*x + (-(x*x)) is nonzero when fused (one rounding leaves a tail), always zero when not
    ...
    // fast/algebraic: exact-semantics execution stays within the allowed set (same value when native cannot reassociate)
    unsafe {
...
fn volatile_mem() {
    ...
    unsafe {
        // Overlapping copy (memmove semantics): shifted 2 elements forward
        ...
        volatile_set_memory(dst.as_mut_ptr(), 0xAB, 2); // elem=u16 -> 4 bytes
        println!("vol set = {dst:?}");
    }
}
...
fn nt_and_mask() {
    ...
    let masked = std::intrinsics::ptr_mask(p, !15usize); // mask down to 16-alignment -> element 16
